...
class SaveLlmConfigParams(BaseModel):
    ...
    qwen_api_key: str = Field(default="", description="Qwen (DashScope) API key — dedicated slot used when provider=qwen, so the shared api_key slot stays free for the default provider (write-only, leave blank to keep)")
